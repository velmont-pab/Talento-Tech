...
OPTION_COLORS = {
    1: Back.MAGENTA + Fore.WHITE,
    2: Back.CYAN + Fore.BLACK,
    3: Back.GREEN + Fore.BLACK,
    4: Back.YELLOW + Fore.BLACK,
}
...
def show_animated_banner(option_text, color, speed=0.02, side_padding=4):
    """
    Muestra el título como un banner ajustado al tamaño del texto con borde, animación y color.
    color: estilo de Colorama para el banner
    """
    os.system("cls" if os.name == "nt" else "clear")
    ...
    print(color + top_bottom + Style.RESET_ALL)

    # Animar línea del texto
    for i in range(1, len(padded_text) + 1):
        line = "|" + padded_text[:i] + " " * (banner_length - i) + "|"
        sys.stdout.write(color + line + Style.RESET_ALL + "\r")
        sys.stdout.flush()
        time.sleep(speed)

    print()  # Salto de línea después del texto
...
def main():
    ...
    while True:
        ...
        if choice == 0 or (
            ...
            color = OPTION_COLORS.get(choice, Back.WHITE + Fore.BLACK)
            show_animated_banner(options[choice - 1], color)
